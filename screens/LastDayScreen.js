...
export default function LastDayScreen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const filtered = secondhands.filter(item => item.lastDay === selectedDay);
      setList(filtered);
    }, [selectedDay])
  );
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerTitle: `Останній день: ${daysOfWeekUa[daysOfWeek.indexOf(selectedDay)]}`,
    });
      ...
      {list.length === 0 ? (
        <Text style={styles.emptyText}>
          {selectedDay === getTodayName()
            ? 'Сьогодні немає останніх днів. Пощастить іншим разом!'
            : `Немає останніх днів на ${daysOfWeekUa[daysOfWeek.indexOf(selectedDay)]}.`
          }
        </Text>
      ) : (
        <FlatList
